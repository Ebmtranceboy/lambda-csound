...
short *buf;
double phi[POLY], phi_mod[POLY], pitch, \
        modulation, velocity[POLY], \
        attack, decay, sustain, release, \
        env_time[POLY], env_level[POLY];
int harmonic, subharmonic, transpose, \
     note[POLY], gate[POLY], note_active[POLY];
unsigned int rate;
int portid;
...
int playback_callback (snd_pcm_sframes_t nframes) {
    ...
    for (l2 = 0; l2 < POLY; l2++) {
        if (note_active[l2]) {
            f1 = 8.176 * exp((double)(transpose+note[l2]-2)* \
                 log(2.0)/12.0);
            f2 = 8.176 * exp((double)(transpose+note[l2])* \
                 log(2.0)/12.0);
            f3 = 8.176 * exp((double)(transpose+note[l2]+2)* \
                 log(2.0)/12.0);
            freq_note = (pitch > 0) ? \
                        f2 + (f3-f2)*pitch : \
                        f2 + (f2-f1)*pitch;
            dphi = M_PI * freq_note / 22050.0;                                    
            dphi_mod = dphi * (double)harmonic / \
                         (double)subharmonic;
            for (l1 = 0; l1 < nframes; l1++) {
                ...
                if (phi_mod[l2] > 2.0 * M_PI) 
                      phi_mod[l2] -= 2.0 * M_PI;
                sound = GAIN * envelope(&note_active[l2], \
                           gate[l2], 
                           &env_level[l2], 
                           env_time[l2], 
                           attack, decay, sustain, release)
                             * velocity[l2] * \
                             sin(phi[l2] + modulation * \
                                    sin(phi_mod[l2]));
