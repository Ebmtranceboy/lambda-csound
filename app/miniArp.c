...
int queue_id, port_in_id, port_out_id, \
     transpose, bpm0, bpm, tempo, swing, \
     sequence[3][MAX_SEQ_LEN], seq_len;
...
snd_seq_t *open_seq() {
  ...
  if (snd_seq_open(&seq_handle, 
     "default", SND_SEQ_OPEN_DUPLEX, 0) < 0) {
    fprintf(stderr, "Error opening ALSA sequencer.\n");
    exit(1);
  }
  snd_seq_set_client_name(seq_handle, "miniArp");
  if ((port_out_id = snd_seq_create_simple_port(seq_handle, 
            "miniFMsynth",
            SND_SEQ_PORT_CAP_READ|SND_SEQ_PORT_CAP_SUBS_READ,
            SND_SEQ_PORT_TYPE_APPLICATION)) < 0) {
    fprintf(stderr, "Error creating sequencer port.\n");
  }
  if ((port_in_id = snd_seq_create_simple_port(seq_handle, 
            "miniFMsynth",
  ...
  snd_seq_connect_to(seq_handle,port_out_id,128,0);
  return(seq_handle);
}
...
void arpeggio() {
  ...
  for (l1 = 0; l1 < seq_len; l1++) {
    dt = (l1 % 2 == 0) ? \
         ...
         -(double)swing / 16384.0;
    snd_seq_ev_clear(&ev);
    snd_seq_ev_set_note(&ev, 0, 
             sequence[2][l1] + transpose, 
             127, 
             sequence[1][l1]);
  ...
  snd_seq_ev_set_dest(&ev, 
        snd_seq_client_id(seq_handle), port_in_id);
  snd_seq_event_output_direct(seq_handle, &ev);
}
    ...
    switch (ev->type) {
      ...
      case SND_SEQ_EVENT_CONTROLLER:
        if (ev->data.control.param == 1) {           
          bpm = (int)((double)bpm0 * \
                 (1.0 + (double)ev->data.control.value / 127.0));
          set_tempo();
        } 
        break;
